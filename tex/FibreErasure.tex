...
used in the [H-color] context~\cite{Beau2026a35,Beau2026a36}.
  ...
  \begin{remark}[Status of~\textup{[H-suff]}]
    ...
    the remaining Born--Infeld gap to the full capacity is open, see~\textup{[O-1a$'$]}.
  \end{remark}
...
$r_c(\ell)$~\cite{Beau2026a1,Beau2026a2,Beau2026a35}.
The open task is to show that the fingerprint projection preserves the kernel factorisation,
\begin{equation}
  \label{eq:bi-kernel}
  p\bigl(\sigma(\ell+1) \mid \sigma(\ell), c\bigr) = p\bigl(\sigma(\ell+1) \mid \sigma(\ell)\bigr),
\end{equation}
so that the fingerprint inherits the $c$-independence already proved for the rank increment.
This is the single gap between the rank result and the full~\textup{[H-suff]} hypothesis,
and the same obstruction left open in the pointwise finite-$q$ form
of~\textup{[H-color]}~\cite{Beau2026a35}.
Numerically, the O32 colour tests support the preservation through the averaged capacity,
the effective exponent, and the finite-$q$ variance collapse~\cite{Beau2026a36}.
Analytically, it suffices to show that the Born--Infeld admissibility weight is invariant
under central-phase permutation at fixed horizontal coset support;
such a lemma would close~\textup{[O-1a$'$]} and the pointwise form of~\textup{[H-color]} at once.

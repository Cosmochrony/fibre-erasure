...
to all $\ell' > \ell$.
  ...
  \paragraph{Result for $q = 61$.}
    ...
    \begin{figure}[h]
      ...
      \caption{Preliminary numerical test of~\textup{[O-2]} for $q = 61$.
      \textbf{Left:} $h_{\mathrm{intra}}(\ell)$, proxy for $H(\sigma(\ell)\mid c)$;
      perfectly monotone non-increasing over the fitting window (blue band),
        zero violations, $t = -25.5$.
  ...
  \paragraph{Role of the multi-$q$ extension.}
    The planned $q \in \{101, 151, 211\}$ tests are not merely robustness checks.
    They should determine whether the decay rate of $h_{\mathrm{intra}}(\ell)$
    is coherent across~$q$, and whether it scales compatibly with the decay
...
Preliminary numerical evidence for the monotonicity is available for $q = 61$
via the intra-block variance proxy (Section~\ref{sec:o2-test});
analytical proof and cross-$q$ confirmation ($q \in \{101, 151, 211\}$)
remain open.

...
% Cover, T.M., Thomas, J.A.: Elements of Information Theory, 2nd ed. Wiley (2006).
  ...
  \section{Introduction}
    ...
    Numerical campaigns across $q \in \{61, 151, 211\}$ reveal a consistent pattern:
    inter-sector variance collapses as depth increases, and the profiles converge
    toward a universal profile~$\sigma^*$.
...
(stated and motivated; see open problem~\textup{[O-1]}).
  ...
  \subsection{Programme placement}
    ...
    as a candidate fibre-erasure fixed point.
...
used in the [H-color] context~\cite{Beau2026a35,Beau2026a36}.
  ...
  A first numerical test is available without new code.
  For each depth step $\ell \to \ell + 1$ in the existing pipeline,
  compute the conditional mutual information $I(c;\,\sigma(\ell+1) \mid \sigma(\ell))$.
  If this quantity is negligible across all tested pairs and depths,
  it constitutes empirical evidence for~\textup{[H-suff]}.
  A non-negligible value at some depth would precisely identify
  the step where the Markov assumption breaks down.
...
to all $\ell' > \ell$.
  ...
  \subsection{Numerical programme}
    ...
    This section will report the numerical results of the fibre-erasure programme
    applied to the existing spectral admissibility data for $q \in \{61, 151, 211\}$.
    Four items are pending numerical computation:
    the inter-sector variance $\mathcal{V}(\ell)$
    and the triplet-restricted variance $\mathcal{V}_{\mathrm{triplet}}(\ell)$
    as functions of BFS depth~$\ell$, with a monotonicity test;
    a direct estimate of~$I(c;\sigma(\ell))$ via a histogram-based
    or $k$-nearest-neighbour mutual-information estimator
    if~$\mathcal{V}$ is found to be non-monotone at some~$q$;
    a test of~\textup{[H-suff]} via the conditional mutual
    information $I(c;\,\sigma(\ell+1) \mid \sigma(\ell))$,
    comparing prediction errors for~$\sigma(\ell+1)$
    with and without conditioning on~$c$;
    and figures showing $\mathcal{V}(\ell)$ and $\mathcal{V}_{\mathrm{triplet}}(\ell)$
    versus~$\ell$ for each~$q$, with colour-triplet curves highlighted.

...
the capacity-level residue is open, see~\textup{[O-1b]}).
  ...
  \subsection{Non-injective projection and fibre structure}
    ...
    The candidate minimal non-injectivity is the parity involution $c \leftrightarrow q - c$,
    a structurally motivated hypothesis (O18 establishes Born--Infeld parity as a covariance
    of the response family, with character $(-1)^{k}$ in the derivative order, and states the
    fibre identification as an open problem): under it, Weil blocks
    $\rho_c$ and $\rho_{q-c}$ map to the same effective observable class.
...
to all $\ell' > \ell$.
  ...
  \subsection{Numerical evidence for~\textup{[H-suff]}}
    ...
    \begin{figure}[h]
      \centering
      \includegraphics[width=\textwidth]{../code/hsuff_vs_depth}
  ...
  \paragraph{Cross-$q$ mean profile comparison.}
    ...
    \begin{figure}[h]
      ...
      \begin{minipage}[t]{0.56\textwidth}
        \includegraphics[width=\textwidth]{../code/o4_cv_profiles}
      \end{minipage}%
      \hfill
      \begin{minipage}[t]{0.41\textwidth}
        \includegraphics[width=\textwidth]{../code/o4_cross_q}
      \end{minipage}
  ...
  \paragraph{Result for $q = 61$.}
    ...
    \begin{figure}[h]
      \centering
      \includegraphics[width=\textwidth]{../code/o2_h_intra}
...
O12's exact rank formula identifies this fingerprint precisely: the increment's rank contribution
depends on the block label $c=(c_1,c_2,c_3)$ only through the projective pair
$(\alpha,\beta)=(B/A,C/A) \bmod q$, where $A=c_1+c_2+c_3$, $B=c_2+c_3$, $C=c_3$ are block-only
constants --- a structural property of the block label itself, not of the visited elements'
central coordinates.
The central coordinate is exactly invisible to the increment: it multiplies each fingerprint
vector by a non-zero unit-modulus scalar and changes neither its frequency nor its span
rank~\cite{Beau2026a16}, so it contributes nothing --- not amplitude, not shape --- to
$\sigma_c(\ell)$.
...
limit)~\cite{Beau2026a36}; at the exact pointwise level on the standard Cayley graph,
$[\mathrm{H\text{-}color}]_{\mathrm{pointwise}}$ is proved analytically in
O31~\cite{Beau2026a35} (Proposition~4.23) via the single-frequency BI fingerprint structure
...
\end{equation}
equivalently that the projective-frequency geometry $(\alpha,\beta)$ contributes only the
amplitude $C_c$, not the shape, of the block decay $\sigma_c(\ell) \sim C_c\, \ell^{-\delta}$.
...
itself a consequence of the $(\alpha,\beta)$ covering geometry --- so it conflates amplitude and
timing with shape and does not establish shape non-universality.
...
The pointwise finite-$q$ form of~\textup{[H-color]} on the standard graph has been closed
in this manner by~O31~v1.5~\cite{Beau2026a35}: the single-frequency BI fingerprint structure
shows that the block's $c$-dependence is carried entirely by the frequency $B$ — preserved by
the orbit dilation $B \mapsto \omega B$ — and not by the central coordinate, and that this
frequency dependence affects only the amplitude, not the shape of the block decay, so capacity
erasure holds pointwise within each colour orbit.
The remaining task for general~$c$, $c'$ (outside the colour orbit) is to extend this
single-frequency argument; this is the residual content of~\textup{[O-1a$'$]}.
...
light cone on the temporal axis~$n$, orthogonal to the present paper.
  ...
  question: does the block's projective-frequency geometry $(\alpha,\beta)$ --- the structural
  property of the exact Weil projection through which its $c$-dependence actually enters, the
  central coordinate itself being exactly inert~\cite{Beau2026a16} --- contribute only the
  block-decay amplitude~$C_c$, not the shape~$\ell^{-\delta}$?
  ...
  so all distinguishability between fibres is carried by the projective-frequency pair
  $(\alpha,\beta)$ that indexes each block's Weil-representation modulation --- a fixed
  structural property of the block label, not a phase accumulated along the walk: the central
  coordinate that makes the group non-abelian is itself exactly inert for this
  observable~\cite{Beau2026a16}.
  Coarse-graining erases the label as the growing BFS ball exhausts every block's finite covering
  geometry alike, not by dephasing or averaging any dynamical quantity: there is no phase for the
  capacity observable to lose, only a bounded combinatorial structure common in kind, if not in
  detail, to every generic block.
